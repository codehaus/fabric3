...
public class ShoalZoneManagerBootstrap {
    ...
    public static void main(String[] args) throws Exception {
        ShoalZoneManagerBootstrap bootstrap = new ShoalZoneManagerBootstrap();
        bootstrap.init();
        while (true) {
            System.out.println("Press <Enter> to dump member cache, 'x' to exit, 's' to send messages to runtime...");
    ...
    public void dumpCache() throws Exception {
        GroupManagementService domainGMS = GMSFactory.getGMSModule("domain");
        for (String name : domainGMS.getGroupHandle().getCurrentCoreMembers()) {
            for (Map.Entry<Serializable, Serializable> entry : domainGMS.getMemberDetails(name).entrySet()) {
    ...
    protected void init() throws Exception {
        ...
        federationService.setRuntimeName("ZoneManager");
        ...
        ShoalRuntimeManager runtimeManager = new ShoalRuntimeManager(federationService, null, null);
        runtimeManager.init();
