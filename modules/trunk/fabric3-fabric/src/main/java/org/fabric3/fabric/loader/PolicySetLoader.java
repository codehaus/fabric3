...
import org.fabric3.spi.model.type.Qualifier;
...
public class PolicySetLoader extends LoaderExtension<Object, PolicySet> {
        ...
        while (true) {
            ...
            case START_ELEMENT:
                ...
                if (INTENTMAP.equals(qname)) {
                    policySet.addIntentMap(loadIntentMap(reader, loaderContext));
                } else if (POLICYSETREFERENCE.equals(qname)) {
                    policySet.addPolicySetReference(loadPolicyReference(reader, loaderContext));
                } else if (WSPOLICYATTACHMENT.equals(qname)) {
                    policySet.addWsPolicyAttachment(loadWSPolicyAttachment(reader, loaderContext));
                }
    ...
    private PolicySetReference loadPolicyReference(XMLStreamReader reader, LoaderContext loaderContext)
            throws XMLStreamException {
    ...
    private IntentMap loadIntentMap(XMLStreamReader reader, LoaderContext loaderContext)
            throws XMLStreamException {
        ...
        while (true) {
            ...
            case START_ELEMENT:
                ...
                if (QUALIFIER.equals(qname)) {
                    intentMap.addQualifier(loadQualifier(reader, loaderContext));
                }
                reader.next();
                break;
            case END_ELEMENT:
    ...
    private Qualifier loadQualifier(XMLStreamReader reader, LoaderContext loaderContext)
            throws XMLStreamException {
        ...
        while (true) {
            ...
            case START_ELEMENT:
                QName qname = reader.getName();
                if (INTENTMAP.equals(qname)) {
                    qualifier.setIntentMap(loadIntentMap(reader, loaderContext));
                } else if (WSPOLICYATTACHMENT.equals(qname)) {
                    qualifier.addWsPolicyAttachment(loadWSPolicyAttachment(reader, loaderContext));
                }
                reader.next();
                break;
            case END_ELEMENT:
    ...
    private WSPolicyAttachment loadWSPolicyAttachment(XMLStreamReader reader, LoaderContext loaderContext)
            throws XMLStreamException {
