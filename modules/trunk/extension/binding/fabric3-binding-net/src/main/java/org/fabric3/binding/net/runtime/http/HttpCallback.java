...
public class HttpCallback implements EncodeCallback {
    ...
    public HttpCallback(HttpRequest request) {
        this.request = request;
    }

    public void encodeContentLengthHeader(long length) {
        request.addHeader(HttpHeaders.Names.CONTENT_LENGTH, String.valueOf(length));
    }

    public void encodeOperationHeader(String name) {
        request.addHeader(NetConstants.OPERATION_NAME, name);
    }

    public void encodeRoutingHeader(String header) {
        request.addHeader(NetConstants.ROUTING, header);
    }
