...
public class JaxWsTargetWireAttacher implements TargetWireAttacher<JaxWsWireTargetDefinition> {
    ...
    throws WiringException {
        ...
        Service service;
        ClassLoader cl = registry.getClassLoader(target.getClassloaderURI());
        assert cl != null;
        ...
        try {
          Class clazz = cl.loadClass(target.getReferenceInterface());
          ...
          for (Map.Entry<PhysicalOperationDefinition, InvocationChain> entry :
            ...
            WsTargetInterceptor wti =
              new WsTargetInterceptor(locateMethod(op, clazz, cl), service, clazz,
