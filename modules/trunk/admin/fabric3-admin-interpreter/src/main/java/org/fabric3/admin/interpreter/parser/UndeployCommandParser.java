...
public class UndeployCommandParser implements CommandParser {
    ...
    private void parseParameter(UndeployCommand command, Iterator<Token> iterator) {
        // proceed past DOWN;
        iterator.next();
        Token token = iterator.next();
        switch (token.getType()) {
    ...
    private void parseContributionName(UndeployCommand command, Iterator<Token> iterator) throws ParseException {
        // proceed past DOWN;
        iterator.next();
